...
class PuttMeter:
	...
	func _ready() -> void:
		...
		custom_minimum_size = Vector2(302, 102)
		queue_redraw()
	...
	func _draw() -> void:
		...
		draw_style_box(_panel(Color(0.045, 0.04, 0.065, 0.94)), bg)
		var font := ThemeDB.fallback_font
		var title := "%s  ·  %s" % [PlayerBadge.glyph(seat), pname]
		draw_string(font, Vector2(16, 30), title, HORIZONTAL_ALIGNMENT_LEFT, -1, 26, pcolor)
		var bar := Rect2(16, 45, size.x - 32, 29)
		draw_rect(bar, Color(0.11, 0.1, 0.15), true)
		for n in range(1, TARGET_SPACES + 1):
			...
			draw_string(font, Vector2(rr.position.x + 2, 68), str(n),
				HORIZONTAL_ALIGNMENT_LEFT, -1, 17, Color(0.08, 0.07, 0.1))
	...
	func _panel(color: Color) -> StyleBoxFlat:
		...
		box.bg_color = color
		box.border_color = Color(pcolor.r, pcolor.g, pcolor.b, 0.85)
